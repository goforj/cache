...
func TestDynamoDriverMethods(t *testing.T) {
	...
	stub := newDynStub()
	store, err := newDynamoStore(ctx, StoreConfig{
		DynamoClient: stub,
		DynamoTable:  "tbl",
		Prefix:       "p",
...
func TestNewDynamoClientError(t *testing.T) {
	...
	if _, err := newDynamoClient(context.Background(), StoreConfig{DynamoRegion: ""}); err == nil {
...
func TestNewDynamoClientWithoutEndpoint(t *testing.T) {
	client, err := newDynamoClient(context.Background(), StoreConfig{DynamoRegion: "us-east-1"})
	if err != nil {
