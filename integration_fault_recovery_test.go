...
func TestIntegrationBackendFaultRecovery_AllDrivers(t *testing.T) {
	if !integrationFaultEnabled() {
		t.Skip("set INTEGRATION_FAULT=1 to run backend restart/recovery integration tests")
	}
...
func assertBackendOutageErrors(t *testing.T, driverName string) error {
	...
	if store.Driver() == DriverDynamo || store.Driver() == DriverSQL {
...
func integrationFaultEnabled() bool {
	v := strings.TrimSpace(strings.ToLower(os.Getenv("INTEGRATION_FAULT")))
